...
foo(c("randomForest", "m2b", "moveHMM", "momentuHMM", "dplyr", "caret","mlbench", "nestr", "coda", "jagsUI", "r2jags", "runjags", "rjags"))
...
chick_rf <- modelRF(xytb, type = "actual", method="ranger", nob = "-1", ntree = 3201, mtry = 40, rfcv = F)
...
xytb.test <- xytb(test.data_1, desc="BTGO Birds",winsize=seq(3,15,2), idquant=seq(0,1,.25))


predict(object=chick_modRF,xytb.test, type = "response", norm.votes = TRUE, predict.all=FALSE, proximity = FALSE, nodes=FALSE,)
